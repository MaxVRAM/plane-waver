...
namespace PlaneWaver
{
    ...
    internal class RandomSystem : ComponentSystem
    {
    ...
    public partial class GrainSynthSystem : SystemBase
    {
        ...
        protected override void OnUpdate()
        {
            ...
            NativeArray<AssetSampleArray> audioClipData = GetEntityQuery(typeof(AssetSampleArray)).ToComponentDataArray<AssetSampleArray>
                    (Allocator.TempJob);
            ...
            JobHandle emitStableGrainsJob = Entities.WithName("EmitStableGrains").WithNativeDisableParallelForRestriction
                    (randomArray).WithReadOnly(audioClipData).WithAll<EmitterReadyTag>().WithNone<EmitterVolatileTag>().ForEach
            ((
                ...
                ref DynamicBuffer<AudioEffectParameters> dspChain) =>
            {
                ...
                float volume = ComputeEmitterParameter(emitter.ModVolume, randomVolume) * emitter.Gain * fadeFactor;
                       ...
                       grainCount < maxGrains)
                {
                    if (volume > 0.005f)
                    {
                        ...
                        ecb.AddComponent
                        (entityInQueryIndex, grainProcessorEntity, new GrainComponent {
                        ...
                        DynamicBuffer<AudioEffectParameters> dspParameters = ecb.AddBuffer<AudioEffectParameters>
                                (entityInQueryIndex, grainProcessorEntity);
                    ...
                    fadeFactor = FadeFactor
                            (sampleIndexNextGrainStart - timer.NextFrameIndexEstimate, emitter.SamplesUntilFade, emitter.SamplesUntilDeath);

                    volume = ComputeEmitterParameter(emitter.ModVolume, randomVolume) * emitter.Gain * fadeFactor;
                }
            ...
            JobHandle emitVolatileGrainsJob = Entities.WithName("EmitVolatileGrains").WithNativeDisableParallelForRestriction
                    (randomArray).WithReadOnly(audioClipData).WithAll<EmitterReadyTag, EmitterVolatileTag>().ForEach
            ((
                int nativeThreadIndex, int entityInQueryIndex, Entity entity, ref DynamicBuffer<AudioEffectParameters> dspChain,
                in EmitterComponent emitter) =>
            {
                var dspTailLength = 0;
                Random randomGen = randomArray[nativeThreadIndex];
                int currentDSPTime = timer.NextFrameIndexEstimate + (int)(randomGen.NextFloat(0, 1) * timer.RandomiseBurstStartIndex);

                var lengthRandom = (int)(randomGen.NextFloat
                                                 (-1, 1) *
                                         emitter.ModLength.Noise *
                                         (emitter.ModLength.Max - emitter.ModLength.Min));

                int totalBurstLength = (int)Mathf.Clamp
                                       (emitter.ModLength.StartValue + emitter.ModLength.ModValue + lengthRandom, emitter.ModLength.Min,
                                           emitter.ModLength.Max) *
                                       samplesPerMS;
                ...
                float volume = ComputeBurstParameter(emitter.ModVolume, offset, totalBurstLength, randomVolume) * emitter.Gain;
                ...
                while (offset < totalBurstLength)
                {
                    ...
                    pitch = Mathf.Pow(2, Mathf.Clamp(transpose, -4f, 4f));
                    volume = ComputeBurstParameter(emitter.ModVolume, offset, totalBurstLength, randomVolume) * emitter.Gain;
                }

                ecb.RemoveComponent<EmitterReadyTag>(entityInQueryIndex, entity);
            }).WithDisposeOnCompletion(audioClipData).ScheduleParallel(emitStableGrainsJob);
            ...
            JobHandle processGrainSamplesJob = Entities.WithName
                    ("ProcessGrainSamples").WithNone<ReflectPlayheadTag, SamplesProcessedTag>().ForEach
            ((
                ...
                for (var i = 0; i < grain.SampleCount - 1; i++)
                {
                    ...
                    if (sourceIndex + 1 >= clipArray.Length)
                    {
                        for (int j = i; j < grain.SampleCount - 1; j++)
                            sampleOutputBuffer.Add
                            (new GrainSampleBufferElement {
                                Value = 0
                            });
            ...
            JobHandle processPingPongSamplesJob = Entities.WithName
                    ("ProcessPingPongSamples").WithAll<ReflectPlayheadTag>().WithNone<SamplesProcessedTag>().ForEach
            ((
                int entityInQueryIndex, Entity entity, DynamicBuffer<GrainSampleBufferElement> sampleOutputBuffer,
                DynamicBuffer<DSPSampleBufferElement> dspBuffer, in GrainComponent grain) =>
            {
            ...
            ((
                DynamicBuffer<AudioEffectParameters> dspParamsBuffer, DynamicBuffer<GrainSampleBufferElement> sampleOutputBuffer,
                DynamicBuffer<DSPSampleBufferElement> dspBuffer, ref GrainComponent grain) =>
            {
        ...
        {
            float random = (mod.UsePerlin
                                   ? mod.PerlinValue
                                   : randomValue) *
                           mod.Noise *
                           Mathf.Abs(mod.Max - mod.Min);
        ...
        private static float ComputeBurstParameter(ParameterComponent mod, float currentSample, float totalSamples, float randomValue)
        {
            ...
            float pathValue = mod.StartValue + timeShaped * (mod.EndValue - mod.StartValue);
            ...
            return Mathf.Clamp(pathValue + modulation + random, mod.Min, mod.Max);
        }
