...
public class DSP_Chopper : DSP_Class
{
    ...
    public static void ProcessDSP(AudioEffectParameters audioEffectParams, DynamicBuffer<GrainSampleBufferElement> sampleBuffer, DynamicBuffer<DSPSampleBufferElement> dspBuffer)
    {
        ...
        for (int i = 0; i < segmentDataStartIndex; i++)
        {
            ...
            {
                rate = audioEffectParams.Value6 * playLength / (audioEffectParams.SampleRate * audioEffectParams.Value0);
            }
            ...
            sampleBuffer[i] = new GrainSampleBufferElement { Value = Mathf.Lerp(sampleBuffer[i].Value, DSP_Utils_DOTS.LinearInterpolate(dspBuffer, sampleToPlay), audioEffectParams.Mix) };
            ...
            if (playIndex >= playLength * Mathf.Floor(audioEffectParams.Value3))
            {
                ...
                // Forward
                if (audioEffectParams.Value4 == 0)
                {
                    playSegment++;
                }
                // Reverse
                else if (audioEffectParams.Value4 == 1)
                {
                    playSegment--;
                }
                // Walk
